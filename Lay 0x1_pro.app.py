...
Original file is located at
    https://colab.research.google.com/drive/1aKbZhzN9XZgWkJmHVrAMj6k-6ZmsqO1O
"""
...
with t1:
    ...
    if st.button("Salvar no log (Pré-jogo)"):
        add_log(dict(
            ...
            score=int(r["score"]),
            signal=r["signal"],
            F1=float(r["F1"]), F2=float(r["F2"]), F3=float(r["F3"]), F4=float(r["F4"]),
            notes=notes
        ))
...
with t2:
    ...
    if st.button("Salvar no log (Live)"):
        add_log(dict(
            ...
            score=int(rl["score"]),
            signal=rl["signal"],
            F1=float(rl["F1"]), F2=float(rl["F2"]), F3=float(rl["F3"]), F4=float(rl["F4"]),
        ...
        if mode == "pregame":
            ...
            if miss:
                ...
                for _, r in df.iterrows():
                    try:
                        rr = pregame_features(safe_float(r["odd_01_open"]), safe_float(r["odd_01_cur"]),
                                              safe_float(r["odd_10_cur"]), safe_float(r["odd_u15_cur"]),
                                              safe_float(r["odd_away_open"]), safe_float(r["odd_away_cur"]))
            ...
            if miss:
                ...
                for _, r in df.iterrows():
                    try:
                        rr = live_features(int(r["minute"]), safe_float(r["odd_01_open"]), safe_float(r["odd_01_live"]),
                                           safe_float(r["odd_10_live"]), safe_float(r["odd_u15_live"]),
                                           safe_float(r["odd_u25_live"]), safe_float(r["odd_away_live"]))
                        row = dict(r); row.update(rr)
                        out.append(row)
                    except Exception:
                        errs += 1
